...
export default function Profile() {
  ...
  const isSelf = !id || id === 'me' || id === me.id;
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 24 }}>
          ...
          <div style={{ minWidth: 0, flex: 1 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <h1 style={{ fontFamily: 'var(--fd)', fontWeight: 600, fontSize: 24, letterSpacing: '-.02em', margin: 0 }}>{me.name}</h1>
              {isSelf && (
            ...
            <div style={{ fontSize: 14, color: 'var(--ink-2)', marginTop: 2 }}>{me.city}</div>
          </div>
        </div>

        {/* Статы: часы · события · надёжность */}
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10, marginBottom: 28 }}>
          <div style={stat}><div style={{ ...statNum, color: 'var(--ink)' }}>{me.hours}</div><div style={statLbl}>{t.profileHours}</div></div>
          <div style={stat}><div style={{ ...statNum, color: 'var(--ink)' }}>{me.events}</div><div style={statLbl}>{t.profileEvents}</div></div>
          <div style={stat}><div style={{ ...statNum, color: 'var(--yard)' }}>{me.reliability}%</div><div style={statLbl}>{t.profileRel}</div></div>
        </div>
