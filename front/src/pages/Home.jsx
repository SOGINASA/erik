...
export default function Home() {
  ...
  // Уже вошёл — на лендинге вместо «Войти/Регистрация» одна кнопка «Войти в систему».
  const enterApp = () => navigate('/feed');
  const enterLabel = isRu ? 'Войти в систему' : 'Жүйеге кіру';
  ...
  const steps = [
    ...
    F(isRu, 'Ответьте одним тапом', 'Бір рет жауап беріңіз', 'Приду · Пока не знаю · Отказаться. Честно и без стыда.', 'Келемін · Білмеймін · Бас тарту. Шынайы әрі ұялмай.'),
      ...
      <header style={{ position: 'sticky', top: 0, zIndex: 30, background: 'rgba(244,245,241,.9)', backdropFilter: 'blur(12px)', WebkitBackdropFilter: 'blur(12px)', borderBottom: '1px solid var(--line)' }}>
        <div style={{ ...wrap, display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: 64 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <LangToggle />
            {loggedIn ? (
              <Button size="sm" onClick={enterApp}>{enterLabel}</Button>
            ) : (
              <>
      ...
      <section style={{ ...wrap, padding: 'clamp(40px,7vw,72px) clamp(20px,4vw,40px) 32px', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 40, alignItems: 'center' }}>
        <div>
          ...
          <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
            {loggedIn ? (
              <Button size="lg" onClick={enterApp}>{enterLabel}</Button>
            ) : (
              <>
      ...
      <section style={{ width: '100%', maxWidth: 1080, margin: '0 auto', padding: '32px clamp(20px,4vw,40px) 72px' }}>
        <div style={{ borderRadius: 'var(--r-l)', background: 'var(--yard)', color: '#fff', padding: 'clamp(32px,5vw,44px) 32px', textAlign: 'center' }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12, justifyContent: 'center' }}>
            {loggedIn ? (
              <button type="button" className="erik-press" onClick={enterApp} style={{ height: 52, padding: '0 28px', border: 'none', borderRadius: 'var(--r-m)', background: '#fff', color: 'var(--yard)', fontWeight: 600, fontSize: 16, cursor: 'pointer', fontFamily: 'var(--fb)' }}>{enterLabel}</button>
            ) : (
              <>
