...
Ключевые эндпоинты прогноза:

```
GET /api/gatherings/<id>/forecast              # прогноз явки (источник: модель, иначе формула)
GET /api/gatherings/<id>/forecast?source=formula   # принудительно фолбэком — показать разницу
GET /api/gatherings/<id>/ml-forecast           # поимённая раскладка модели по участникам
GET /api/forecast/quality                      # паспорт модели: метрики, базлайны, важность признаков (публично)
```

Прогноз также приезжает вместе со сбором (`GET /api/gatherings/<id>` → `gathering.forecast`),
в каждом тике поллинга (`/poll` → `forecast`) и в карточках штаба (`/me/org/events` →
`events[].forecast`). Отдельный запрос за числом фронту делать не нужно.

## Прогноз явки

**Основной источник — обученная ML-модель** (`../ml`, мост
[`services/attendance_ml.py`](services/attendance_ml.py)): градиентный бустинг на 14
причинных признаках с изотонической калибровкой, `p_i = P(придёт)`, прогноз `E = Σ p_i`.
Историю для признаков собирает [`services/history_features.py`](services/history_features.py)
из журнала `AttendanceRecord` — только по сборам, прошедшим раньше текущего.

**Фолбэк — аналитическая формула** в [`services/forecast.py`](services/forecast.py): для
каждого участника `p_i = clamp(base(answer)·trust_i·ctx)`, где `trust_i` — доля явок со
сглаживанием Лапласа (α из `ForecastParams`). Прогноз `E = Σ p_i`, интервал `E ± 2σ`.
Параметры (`base`, `alpha`, `sigma_k`, границы) хранятся в таблице-синглтоне `ForecastParams` — чтобы
переоценивать их на реальных данных, а не хардкодить.

Мост грузит модель один раз и **мягко деградирует**: нет ml-зависимостей или артефакта —
ответ приходит с `source: "formula"` и заполненным `fallbackReason`, сервер не падает и
код 200 сохраняется. Источник выбирается **целиком на сбор**: смешивать ML-вероятности
одних участников с формульными других запрещено, иначе по числу нельзя понять, чем оно
посчитано.
...
| переменная | смысл |
|---|---|
| `ERIK_ML_DIR` | путь к каталогу `ml/` (в Docker — `/ml`) |
| `ERIK_ML_DISABLE=1` | погасить модель и проверить фолбэк, не трогая код |
| `ERIK_ML_WARMUP=1` | прогреть модель при старте воркера (иначе первый запрос платит распаковку joblib) |

Прогноз кэшируется по отпечатку ростера `(id, revision, needed, [(participant_id, answer)])` —
`/poll` дёргается раз в 10 секунд на каждого координатора, и без кэша каждый тик был бы
проходом модели по всему ростеру.
