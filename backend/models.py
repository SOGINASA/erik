...
class Gathering(db.Model):
    ...
    status = db.Column(db.String(8), default='pending')   # pending (на модерации) | open | done | deleted | rejected
    reject_reason = db.Column(db.String(400), nullable=True)  # причина отклонения модерацией (status='rejected')
